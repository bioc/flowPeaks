...
Experienced R users need to carefully follow the instruction of the Rtools (\url{http://www.murdoch-sutherland.com/Rtools/}) and \url{http://cran.r-project.org/doc/manuals/R-admin.html#The-Windows-toolset}. The GSL library needs to be downloaded from the file \texttt{local.zip} at 
\url{http://www.stats.ox.ac.uk/pub/Rtools/goodies/Win32} and the \texttt{GSL\_LIB} in the file
\texttt{flowPeaks/src/Makevars.win}  should be pointed to the top folder of the extracted file \texttt{local.zip}. The top folder should contain three subfolders: include, bin and lib. The users are not encouraged to compile their own gsl library by MinGW or Visual Studio. Most likely their own version of gsl library is not going to work. 
...
\begin{enumerate}
  ...
  \item  In the file \texttt{flowPeaks/src/Makevars}, you may need to change the last two lines as below:
    \begin{itemize}
    \item  \texttt{PKG\_CXXFLAGS = \$(GSL\_CFLAGS) -I. -I<path-to-include>}
    \item  \texttt{PKG\_LIBS = \$(GSL\_LIBS) -L<path-to-lib>}
    \end{itemize}
\end{enumerate}
