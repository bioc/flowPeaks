...
/* 
* This code was originally written by Stephan Fortune in C code and modified 
* into C++ by Shane O'Sullivan. I, Yongchao Ge, have modified it to use in R
* Permission to use, copy, modify, and distribute this software for any
* purpose without fee is hereby granted, provided that this entire notice
* is included in all copies of any software which is or includes a copy
* or modification of this software and in all copies of the supporting
...
//providing R interfaces
void voronoi(int *n, double *xv, double *yv, int* autobox, double*box,//input
	     //xv and yv are size n, box is size 4 if autobox is true
	     //coordv are size nedge*4, sizev are size nedge*2
	     int *nedge, double *coordv, int *sitev)//output
//need to allocated the corresponding nedge=3*n-6; for n>=3;
{
    int count=*n;
    double xybox[4],xyrng[2];
    const double eps=1e-7;
    xybox[0]=*min_element(xv,xv+count);
    xybox[1]=*max_element(xv,xv+count);
    xybox[2]=*min_element(yv,yv+count);
    xybox[3]=*max_element(yv,yv+count);
    xyrng[0]=xybox[1]-xybox[0]+eps;
    xyrng[1]=xybox[3]-xybox[2]+eps;
   
    double del=eps;
    double coef[4]={xyrng[0],xyrng[0],xyrng[1],xyrng[1]};
    int dir[4]={-1,1,-1,1};

    {
	for(int i=0;i<4;i++){
	    xybox[i]+=dir[i]*coef[i]*0.33;
	}
    }
    if(!*autobox){
	/*double mul[2]={0.1,10};
	int mul_id[4]={1,0,1,0};
	for(int i=0;i<4;i++){
	    double xymin=xybox[i]+mul[mul_id[i]]*dir[i]*coef[i];
	    double xymax=xybox[i]+mul[1-mul_id[i]]*dir[i]*coef[i];
	    double xy=box[i];
	    if(xy>=xymax){
		xy=xymax;
	    }else if(xy<=xymin){
	...
	for(int i=0;i<4;i++){
	    if( (xybox[i]-box[i])*dir[i]<0) xybox[i]=box[i];
	}
    }
    
    VoronoiDiagramGenerator vdg;
    vdg.generateVoronoi(xv,yv,count,box[0],box[1],
			box[2],box[3],del);
    vdg.resetIterator();
    ...
    int s1_id,s2_id;
    int ne=0;
    while(vdg.getNext(x1,y1,x2,y2,s1_id,s2_id))
    {
	coordv[ne*4]=x1;
	coordv[ne*4+1]=y1;
	coordv[ne*4+2]=x2;
...
VoronoiDiagramGenerator::VoronoiDiagramGenerator()
{
	...
	allMemoryList = mycalloc(1,FreeNodeArrayList);
...
char * VoronoiDiagramGenerator::getfree(struct Freelist *fl)
{
	...
	{	
		...
		currentMemoryBlock->next = mycalloc(1,FreeNodeArrayList);
...
char * VoronoiDiagramGenerator::myalloc_vor(unsigned n)
{
	char *t=0;	
	t=mymalloc(n,char);//(char*)malloc(n);
	total_alloc += n;
	return(t);
}
...
void VoronoiDiagramGenerator::line(double x1, double y1, double x2, double y2,
				   int s1_id, int s2_id)
{	
    pushGraphEdge(x1,y1,x2,y2,s1_id,s2_id);
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
	...
	newintstar.y=0; //probably useless.
	int pm;
